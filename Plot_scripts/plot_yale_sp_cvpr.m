function [] = plot_yale_sp_cvpr(noise_level, raw)
...
load(to_load)
[measures.rpca, best.rpca] = distilate(results_rpca, 'lambda', 0);
clear results_rnndl
...
to_load = sprintf('/vol/bitbucket/mb2215/Thesis/bilinear-rpca/results_%s_%s_%s_%f.mat', dataset, method, noise_type, noise_level);
load(to_load)
[measures.rnndl, best.rnndl] = distilate(results_rnndl, 'lambda', 0);
clear results_rnndl
...
title(sprintf('PSNR Yale - %2.0f%% noise', 100*noise_level));
...
title(sprintf('FSIM Yale - %2.0f%% noise', 100*noise_level));
xlabel('First parameter')
ylabel('FSIM')
% xlim([2e-4, 5e-1]);
legend(legend_names{:}, 'location', 'northeastoutside');
...
file = my_sprintf('fsim_%s_%s_%f_cvpr.pdf', dataset, noise_type, noise_level);
...
%% Compute means and stuff

[O, ~] = yale_sp(1, 0.1);
O = O(:,:,1:64);
O = rescale(O, 0, 255);

list_psnrs = containers.Map;
list_fsims = containers.Map;
mean_psnrs = containers.Map;
mean_fsims = containers.Map;
sd_psnrs = containers.Map;
sd_fsims = containers.Map;

for i=1:n_1d
    n = algorithms{i};
%     fprintf('Processing %s\n', n);
    [psnrs, fsims] = get_all_measures(best.(n), O);
    list_psnrs(n) = psnrs;
    list_fsims(n) = fsims;
    mean_psnrs(n) = mean(psnrs);
    mean_fsims(n) = mean(fsims);
    sd_psnrs(n) = std(psnrs);
    sd_fsims(n) = std(fsims);
    fprintf('%s, Mean PSNR, %f, Std PSNR, %f, Mean FSIM, %f, Std FSIM, %f,\n', ...
        n, mean_psnrs(n), sd_psnrs(n), mean_fsims(n), sd_fsims(n));
end
for i=1:n_2d
    j = n_1d + i;
    n = algorithms_2d{i};
%     fprintf('Processing %s\n', n);
    [psnrs, fsims] = get_all_measures(best_2d.(n), O);
    list_psnrs(n) = psnrs;
    list_fsims(n) = fsims;
    mean_psnrs(n) = mean(psnrs);
    mean_fsims(n) = mean(fsims);
...
save('means_sd.mat', 'list_psnrs', 'list_fsims', 'mean_psnrs', 'mean_fsims', 'sd_psnrs', 'sd_fsims');

fprintf('Done\n');

end

function [psnrs, fsims] = get_all_measures(structure, O)
psnrs = zeros(1, 64);
fsims = zeros(1, 64);
Xpsnr =rescale(structure.psnr.L, 0, 255);
Xfsim =rescale(structure.fsim.L, 0, 255);

for i = 1:64
    msiqa = MSIQA(O(:,:,i), Xpsnr(:,:,i));
    psnrs(i) = msiqa.psnr;
    msiqa = MSIQA(O(:,:,i), Xfsim(:,:,i));
    fsims(i) = msiqa.fsim;
end
