...
results_welsh_st = cell(length(sigma), length(alpha));
i = 1;
...
%             [D, I] = BRTF(X, k, nFrames, 'initVar', a, 'maxRank', 168);
            [L, iter,ranks] = lrtc_LiBCD_GS(X, H, X_cell, a*lambdaS,  s, 1, @grad_mcc, @sv_shrinkage, eps, max_iter);
            time = toc;
            S = X - L;  % Heuristic
            ...
            results_welsh_st{i,j} = exception;
            fprintf('EXCEPTION: %s\n', exception.message);
        end
