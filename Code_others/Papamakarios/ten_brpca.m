function [Y, info] = ten_brpca(X, options)
% [Y, info] = ten_brpca(X, options)
% Performs Bilinear Robust Principal Component Analysis on tensor X.
% INPUTS
...
if ~isfield(options, 'initU')  
    ...
    for i = 1:N
        options.initU{i} = rand(dims(i), options.numComp(i));
    end
end
if ~isfield(options, 'initA')  
    options.initA = tenrand(dims); 
end
...
while err > termCrit && iter < options.maxIter
    ...
    sumJ = tenzeros(dims);
    for i = 1:N
        AL2m = double(tenmat(A,i) + L2{i}*(1/m));
        V{i} = (U{i}'*U{i} + a(i)*eye(k(i))/m) \ U{i}' * AL2m;
        U{i} = AL2m * V{i}' / (V{i}*V{i}' + a(i)*eye(k(i))/m);
        J{i}(:,:) = U{i} * V{i};
