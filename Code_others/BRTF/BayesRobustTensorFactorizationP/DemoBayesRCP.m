...
tic
[model] = BayesRCP(Y, 'init', 'ml', 'maxRank', max([DIM,2*R]), 'maxiters', 100, 'initVar', 1, 'updateHyper', 'on',...
'tol', 1e-5, 'dimRed', 1, 'verbose', 2);
t_total = toc;
...
if 0
    opts.noARDiter=25;
    opts.method='Sparse';    % 'Sparse'  'Ridge'
    opts.constrFACT=zeros(1,ndims(Y));
    opts.constrCore=0;
    opts.constCore = 1;
    [Core_est, FACT_est, alpha_est,varexpl] = ARDTUCKER(Y, max(DIM).*[1 1 1], opts);
    
    X_hat =reconstructTucker(Core_est,FACT_est);
    err = X_hat(:) - X(:);
    rmse = sqrt( mean(err.^2));
    rrse = sqrt(sum(err.^2)/sum(X(:).^2));
    fprintf('\n-------------ARD Tucker-----------------------------------------\n')
    fprintf('RRSE = %g, RMSE = %g, rank = %g \n', rrse, rmse, size(FACT_est{1},2));
    fprintf('-------------------------------------------------------------\n')
end
