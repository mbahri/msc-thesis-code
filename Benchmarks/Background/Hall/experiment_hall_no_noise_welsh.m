...
for ni = 1:length(numims)
    ...
    filename = sprintf('%dx%dx%d_no_noise_welsh', size(thisX));
...
%     end
    ...
    params_mest.p = sigma;
    params_mest.q = 1;
    results_welsh.welsh_st = benchmark_mod(@geo_welsh_st, data, params_mest, true, GT_frames);
    
    save(file, 'results_welsh', '-v7.3');
end

% save parameters
paramsfile = fullfile(rootdir, 'params_no_noise_welsh.mat');
