...
for ni = 1:length(numims)
    ...
    filename = sprintf('%dx%dx%d_no_noise_nctrpca', size(thisX));
...
%     end
    ...
    % NCTRPCA
    rankk = floor(linspace(5, 48, 10));
    thr = logspace(1, 4, 4);
    
    params_nctrpca.lambda = rankk;
    params_nctrpca.p = thr;
    params_nctrpca.q = 1;
    results_nctrpca.nctrpca = benchmark(@geo_nctrpca, data, params_nctrpca, true);
    
    save(file, 'results_nctrpca', '-v7.3');
end

% save parameters
paramsfile = fullfile(rootdir, 'params_no_noise_nctrpca.mat');
