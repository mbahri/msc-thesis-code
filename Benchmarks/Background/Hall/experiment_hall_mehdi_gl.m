...
for ni = 1:length(numims)
    ...
    filename = sprintf('%dx%dx%d_no_noise_gl', size(thisX));
...
%     results.tnn = benchmark(@geo_trpca_tnn, data, params_tnn, true);
    ...
    % 2D RPCA
    data.X = X; data.A = X;
    params_2d_gl.lambda = 2*logspace(-1, -4, 20);
    params_2d_gl.p = logspace(-1, -4, 10);
    params_2d_gl.q = logspace(-1, -4, 10);
    
    %all_rpca2d = ...
    %    {@rpca2d_gl_l2, @rpca2d_gl_l1};
    %all_rpca2d_names = ...
    %    {'rpca2d_gl_l2', 'rpca2d_gl_l1'};

    all_rpca2d = ...
        {@rpca2d_gl_l1, @rpca2d_gl_l2};
    all_rpca2d_names = ...
        {'rpca2d_gl_l1', 'rpca2d_gl_l2'};
    ...
    for i=1:length(all_rpca2d)
        ...
        alg_rpca2d = @(X, opt)(geo_rpca2d_highway_gl(all_rpca2d{i}, X, opt));
...
% save parameters
paramsfile = fullfile(rootdir, 'params_no_noise_gl.mat');
