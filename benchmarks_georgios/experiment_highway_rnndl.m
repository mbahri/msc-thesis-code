...
for ni = 1:length(numims)
    ...
    filename = sprintf('%dx%dx%d_no_noise_rnndl', size(thisX));
...
% % Add all other algorithms
    ...
    % RNNDL
    data.X = X; data.A = X;
    params_rnndl.lambda = 0:0.5:5;
    params_rnndl.p = 1; params_rnndl.q = 1;
    results_rnndl = benchmark(@geo_rnndl, data, params_rnndl, true);
    
    save(file, 'results_rnndl', '-v7.3');
end

% save parameters
paramsfile = fullfile(rootdir, 'params_no_noise_rnndl.mat');
