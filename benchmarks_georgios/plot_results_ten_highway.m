...
% 
% % figure save path
% savepath = fullfile('/vol/bitbucket/mb2215/Thesis', 'results', 'highway');
% 
% % squeeze matrices
...
% saveas(fig, fullfile(savepath, 'ten_highway_time.eps'), 'epsc');
...
print(fig, '-depsc', fullfile(savepath, 'ten_highway_time'));
...
%ylim([1.0e-7 1.0e+2]);
set(fig, 'PaperPositionMode', 'auto'); 
% saveas(fig, fullfile(savepath, 'ten_highway_performance.eps'), 'epsc');
...
fig.PaperUnits = units;
fig.PaperPosition = paperposition;
print(fig, '-depsc', fullfile(savepath, 'ten_highway_performance'));
...
% ylim([1.0e-7, 1]);
% set(fig, 'PaperPositionMode', 'auto'); 
% % saveas(fig, fullfile(savepath, 'ten_highway_convergence.eps'), 'epsc');
% 
% fig.PaperOrientation = 'landscape';
% fig.Renderer = 'painters';
% fig.PaperUnits = 'centimeters';
% fig.PaperPosition = [0 0 29.7 21];
% print(fig, '-dpdf', fullfile(savepath, 'ten_highway_convergence.pdf'));
...
fprintf('RPCA2D GL L2 (Bahri et. al.) & %.4f & %.3g & %.3g & %u & %.2f\\\\ \\hline\n', ...
    best_auc('rpca2d_gl_l2'), params_2d_gl.lambda(best_l_idx('rpca2d_gl_l2')), params_2d_gl.p(best_r_idx('rpca2d_gl_l2')), ...
    results.rpca2d_gl_l2.info{best_l_idx('rpca2d_gl_l2'), best_r_idx('rpca2d_gl_l2')}.iter(end), ...
    results.rpca2d_gl_l2.time(best_l_idx('rpca2d_gl_l2'), best_r_idx('rpca2d_gl_l2')) );

fprintf('RPCA2D GL L1 (Bahri et. al.) & %.4f & %.3g & %.3g & %u & %.2f\\\\ \\hline\n', ...
    best_auc('rpca2d_gl_l1'), params_2d_gl.lambda(best_l_idx('rpca2d_gl_l1')), params_2d_gl.p(best_r_idx('rpca2d_gl_l1')), ...
    results.rpca2d_gl_l1.info{best_l_idx('rpca2d_gl_l1'), best_r_idx('rpca2d_gl_l1')}.iter(end), ...
    results.rpca2d_gl_l1.time(best_l_idx('rpca2d_gl_l1'), best_r_idx('rpca2d_gl_l1')) );
